...
//---------function for  creating & sending signToken   ----------

const createSendToken = (user, statusCode, res) => {
  const token = signToken(user._id);
  //------------COOKIE SECTION-------------
  //we can see cookie in signup (development mode)
  const cookieOptions = {
    expires: new Date(
...
//*************FOR forntEnd************
//Only for rendered pages (frontend log-btn)
exports.isLoggedIn = catchAsync(async (req, res, next) => {
  if (req.cookie.jwt) {
    ...
    // 2) Check if user still exists (expires time)
    const currentUser = await User.findById(decoded.id);
    if (!currentUser) {
      return next();
    }

    // 3) Check if user changed password after the token was released
    if (currentUser.changedPasswordAfter(decoded.iat)) {
      return next();
    }
...
//-----------Authorization: User Roles and Permissions----------------(admin,lead-guide has permissiion to delete)
